...
class EndpointSelector:
    ...
    def run(self) -> None:
        ...
        JsonFile.write_json(
            selected_oas,
            file_path="docs/selected-coingecko"
        )
        ColorPrint.print_success(f"Selected OAS at {RootPath.get_package_root() / 'docs/selected-coingecko.json'}")
    ...
    def select_endpoints(
        ...
        ColorPrint.print_info(f"Selected {len(selected_paths)} paths: {len(selected_onchain)} from /onchain")
    ...
    def extract_relevant_schemas(
        ...
        ColorPrint.print_info(f"Selected {len(selected_schemas)} schemas")
